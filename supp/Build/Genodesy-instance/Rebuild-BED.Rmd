...
config <- lapply(
  config, sub, pattern="___HOME___", replacement=Sys.getenv("HOME")
)
config <- lapply(
  config, sub, pattern="___ROOT___", replacement=config$ROOT
)
##
...
## https://ftp.ensembl.org/pub/release-111/mysql/ (use the latest release)
## Search for species_core (e.g. mus_musculus_core)
## get the gv (genome version) just after the release number
...
BED:::getUniprot(
    organism="Danio rerio", release=avRel, ddir=wd
)
gc()
```
...
BED:::getUniprot(
    organism="Homo sapiens", release=avRel, ddir=wd
)
gc()
```
...
BED:::getUniprot(
    organism="Mus musculus", release=avRel, ddir=wd
)
gc()
```
...
BED:::getUniprot(
    organism="Rattus norvegicus", release=avRel, ddir=wd
)
gc()
```
...
## Import plateform
platName <- "GPL1708"
gds <- getGEO(platName, destdir=file.path(wd, "geo"))
