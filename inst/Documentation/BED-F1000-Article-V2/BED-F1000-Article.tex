...
\begin{abstract}
The understanding of molecular processes involved in a specific biological system can be significantly improved by combining and comparing different data sets and knowledge resources. However these information sources often use different identification systems and an identifier conversion step is required before any integration effort. Mapping between identifiers is often provided by the reference information resources and several tools have been implemented to simplify their use. However, most of these tools do not combine the information provided by different resources to increase the efficiency of the mapping process and deprecated identifiers from former version of databases are not taken into account. Finally finding automatically the most relevant path to map identifiers from one scope to the other is often not trivial. The Biological Entity Dictionary (BED) addresses these challenges by relying on a graph data model describing possible relationships between entities and their identifiers. This model has been implemented using Neo4j and an R package provides functions to query the graph but also to create and feed a custom instance of the database. This design combined with a local installation of the graph database and a cache sytem make BED very efficient to convert large lists of identifiers.
\end{abstract}
...
All BEID from a specific scope can be get with the \texttt{getBeIds} function.
...
levels of granularity. For exemple, Ensembl \citep{zerbino_ensembl_2017} provides
possible transcripts for each gene specifying which of them is canonical.
The \texttt{getDirectProduct} and \texttt{getDirectOrigin} function allow the user
...
or on data frames with an identifer column (\texttt{convDfBeIds}).
...
All these use cases were executed using the instance of the BED database
described in Methods and which is available
...
\caption{Barplots showing the number of gene BE (log scale) identified by one or more Ensembl gene BEID. a) All Ensembl gene ID. b) Current Ensembl gene ID (version 92).}\label{fig:beidTables}
\end{figure}
...
Three other tools were used on May 03, 2018
...
\begin{itemize}
\item
  Reference (N = 19,166): the mappings
  provided by biomaRt for identifiers successfully converted by all the tools.
\item
  Former (N = 67): the mappings
  provided by mygene for identifers
  successfully converted by mygene but neither by biomaRt nor BED.
\item
  BED (N = 12,913): the mappings
...
\caption{Comparison of gene locations provided by Ensembl for Ensembl identifiers and by the NCBI for the cross-referenced Entrez identifiers. a) Proportion of cross-referenced identifiers located on different chromosomes. b) Absolute distance (base pairs) between gene start positions for cross-referenced identifiers located on the same chromosome. c) Absolute difference (base pairs) between gene sizes for cross-referenced identifiers located on the same chromosome. Median and third quartile are indicated. Outliers are not shown. BED and Former results are compared to Reference on different boxplots because of scale shift.}\label{fig:geneLocations}
\end{figure}
...
These results shows that the general quality of former mappings is lower
than the reference. More interestingly, additional mappings provided by BED
are of similar quality to the reference.

\subsection{Transitivity and mapping rules}\label{transitivity-and-mapping-rules}
...
On the other hand a BEID which \emph{is\_associated\_to} to another one does not
...
For example, in the instance used for executing these use cases,
...
to convert identifiers accross different type of BE.
...
For example
the (A6NI28){[}\url{http://www.uniprot.org/uniprot/A6NI28}{]} Uniprot identifier
...
((ENSP00000431776){[}\url{http://www.ensembl.org/id/ENSP00000431776}{]},
(ENSP00000434304){[}\url{http://www.ensembl.org/id/ENSP00000434304}{]}
and (ENSP00000435961){[}\url{http://www.ensembl.org/id/ENSP00000435961}{]}
which are encoded by
the same gene ((ENSG00000165895){[}\url{http://www.ensembl.org/id/ENSG00000165895}{]})).

Finally mapping BEID which are not genes from two different
organisms using ortholog information requires at least two steps in biomaRt,
mygene and gProfileR: one for finding the ortholog genes and the other
for finding the relevant BEID.
...
\subsection{Comparing transcriptomics results from different experimental design}\label{comparing-transcriptomics-results-from-different-experimental-design}

The mechanisms implemented in the BED R package make easy the
...
is straigtforward from a technical point of view.

To examplify this statement we show below how to compare differential expression
data from three experiments adressing the understanding of Psoriais mecahnisms
...
\item
  ...
  ((E-GEOD-13355){[}\url{https://www.ebi.ac.uk/arrayexpress/experiments/E-GEOD-13355/}{]})
...
\item
  ...
  Starting from published data
  ((E-GEOD-24767){[}\url{https://www.ebi.ac.uk/arrayexpress/experiments/E-GEOD-24767/}{]})
...
\item
  ...
  Starting from published data
  ((E-GEOD-27628){[}\url{https://www.ebi.ac.uk/arrayexpress/experiments/E-GEOD-27628/}{]})
  we recomputed differential expression between affected and non-affected
  skin samples.
\end{itemize}

The head of these three data tables is shown below.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## E-GEOD-13355: Human skin}
\KeywordTok{head}\NormalTok{(hsSkin.DE)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                logFC   AveExpr        t      P.Value    adj.P.Val        B
## 41469_at    5.140278 0.5523489 40.78083 8.384147e-46 4.584033e-41 93.83004
## 232170_at   5.766759 0.5624087 39.83855 3.293084e-45 9.002469e-41 92.51495
## 205863_at   4.715087 0.4439447 39.45503 5.796620e-45 1.056434e-40 91.97049
## 203691_at   5.396590 0.6140546 39.15392 9.068285e-45 1.239521e-40 91.53922
## 217388_s_at 3.926282 0.3818259 38.05322 4.785688e-44 5.233150e-40 89.93339
## 207356_at   5.806316 0.6525531 37.57034 1.006845e-43 9.174875e-40 89.21400
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## E-GEOD-24767: Human keratinocytes}
\KeywordTok{head}\NormalTok{(hsKera.DE)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                 logFC   AveExpr        t      P.Value    adj.P.Val
## ILMN_2048043 6.297295  8.214566 68.44411 2.186104e-14 1.066841e-09
## ILMN_1672295 3.507524  8.536838 38.47825 5.823956e-12 1.051466e-07
## ILMN_1680965 6.114791  8.682000 38.06585 6.463797e-12 1.051466e-07
## ILMN_1799575 6.093054  7.921023 19.62628 3.759130e-09 4.586232e-05
## ILMN_1795359 4.930397 10.920341 18.17466 7.797664e-09 6.838764e-05
## ILMN_1757351 2.554070  6.793824 18.03065 8.408145e-09 6.838764e-05
##                      B
## ILMN_2048043 11.989637
## ILMN_1672295 11.327400
## ILMN_1680965 11.307684
## ILMN_1799575  9.253457
## ILMN_1795359  8.888501
## ILMN_1757351  8.849106
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## E-GEOD-27628: Mouse skin}
\KeywordTok{head}\NormalTok{(mmSkin.DE)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                 logFC  AveExpr         t      P.Value   adj.P.Val        B
## 1440888_at -1.3929330 6.980290 -7.325822 3.317833e-08 0.000732987 8.677033
## 1449319_at -0.9672017 6.303944 -7.247096 4.105579e-08 0.000732987 8.484408
## 1422803_at -0.7688354 8.452405 -7.113243 5.906985e-08 0.000732987 8.154970
## 1435272_at -0.7092831 9.788428 -7.031605 7.381416e-08 0.000732987 7.952878
## 1428499_at -0.7661095 7.306794 -6.996465 8.126241e-08 0.000732987 7.865623
## 1416084_at  0.5313674 9.273609  6.724232 1.718620e-07 0.001115532 7.184418
\end{verbatim}
...
Without using its cache system, BED performs equaly well as biomaRt for
converting Ensembl human gene ID into Entrez gene ID
(Figures \ref{fig:perf}.a and \ref{fig:perf}.b).
It ouperforms biomaRt for converting 100 Uniprot mouse peptide ID
...
research project, BED can be advantageously employed by sytems dealing with
biological and molecular information from heterogenous sources.
...
will encourage other developpers to extend or improve it according to their
